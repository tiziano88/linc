...
deleteNodeVariableDefinition : ExprRef -> Ast.VariableDefinition -> Ast.VariableDefinition
deleteNodeVariableDefinition ref def =
    { def
        | value = Maybe.map (deleteNodeExpression ref) def.value
    }


deleteNodeExpression : ExprRef -> Ast.Expression -> Ast.Expression
deleteNodeExpression ref expr =
    let
        newValue =
            case expr.value of
                Ast.IfValue v1 ->
